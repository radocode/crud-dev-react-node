...
class Loginscreen extends Component {
  constructor(props) {
    ...
    loginButtons.push(
      <div key={"Login-Div"}>
        <MuiThemeProvider>
          <div>
            <RaisedButton label={"Registrarse como Usuario"} primary={true} style={style} onClick={(event) => this.handleClick(event, 'user')} />
          </div>
        </MuiThemeProvider>
        <MuiThemeProvider>
          <div>
            <RaisedButton label={"Registrarse como Admin"} primary={true} style={style} onClick={(event) => this.handleClick(event, 'admin')} />
          </div>
        </MuiThemeProvider>
      </div>
    )
    this.state = {
      ...
      userbuttonLabel: 'Registrarse como Usuario',
      adminbuttonLabel: 'Registrarse como Admin',
      isLogin: true
    }
  }
  componentWillMount() {
    ...
    var loginmessage = "Aun no registrado? Registrese aqui";
    this.setState({
  ...
  handleClick(event, userRole) {
    ...
    if (this.state.isLogin) {
      ...
      loginmessage = "Ya tiene cuenta? Ir a Login";
      let loginButtons = [];
      loginButtons.push(
      ...
      loginmessage = "Aun no se ha registrado. Ir a registro";
      this.setState({
        loginscreen: loginscreen,
        loginmessage: loginmessage,
        loginButtons: loginButtons,
